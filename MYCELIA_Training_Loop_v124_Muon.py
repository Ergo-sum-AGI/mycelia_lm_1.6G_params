# ============================================
# MYCELIA Training Loop — v12.4 (1.5B Muon Integration)
...
# v11.9: Clean resume block, single LOG_EVERY gate, orphan code removed
# v12.0: Connect internal meta-governor
# v12.4: Added Surfer
# ============================================
...
# from meta_governor import integrate_meta_governor -> replaced with internal gov.
...
GRAD_CLIP = 2.0
SAVE_EVERY = 1000
LOG_EVERY = 20   # was 250 an after that was 50 and then 100
...
def compute_spectral_concentration(hidden_states: torch.Tensor, top_k: int = 3) -> float:
    """Compute spectral concentration as ratio of top-k singular values to total singular value mass."""
    ...
    try:
        ...
        if total < 1e-8:
            return 0.0
        top_k_sum = S[:min(top_k, len(S))].sum()
        concentration = (top_k_sum / total).item()
...
def _safe_load_checkpoint(path):
    """Load checkpoint with fast CPU-first path to avoid GPU OOM."""
    ...
    # v12.2: ALWAYS load to CPU first. GPU memory is already full with model + optimizer.
    # Large checkpoints bypass weights_only=True (prohibitively slow on 9GB+).
    use_safe = file_size < 5e9

    try:
        t0 = time.time()
        print(f"   ⏳ Loading to CPU... (est. {max(30, int(file_size/3e8))}s)")
        sys.stdout.flush()

        if use_safe:
            try:
                ckpt = torch.load(path, map_location='cpu', weights_only=True)
                print(f"   ✅ Safe CPU load")
            except Exception:
                print(f"   ⚠️  Safe load failed, using legacy...")
                ckpt = torch.load(path, map_location='cpu', weights_only=False)
                print(f"   ✅ Legacy CPU load")
        else:
            ckpt = torch.load(path, map_location='cpu', weights_only=False)
            print(f"   ✅ Fast CPU load (weights_only=False)")
    ...
    try:
        t0 = time.time()
        print(f"   ⏳ Loading to CPU... (est. {max(600, int(file_size/3e8))}s)")
        sys.stdout.flush()
...
# ============================================
print("\n" + "="*70)
print("🍄 MYCELIA TRAINING v11 (1.5B Muon Integration)")
...
if ckpt is not None:
    ...
    for k, v in ckpt_state.items():
        if k in model_state and model_state[k].shape != v.shape:
            if '_prev_hidden' in k or '_prev_delta' in k:
                legacy_buffers += 1
                continue
    ...
    if legacy_buffers > 0:
        print(f"   🧹 Silently reset {legacy_buffers} legacy geometric buffers")
    if skipped > 0:
    ...
    if 'auto_tuner_state' in ckpt:
        try:
            ...
            print("   ✅ Auto-tuner restored")
        except Exception as e:
            print(f"   ⚠️  Auto-tuner restore failed: {e}")
            
    # v12.1: Force meta-governor reset on resume (API -> internal transition)
    if hasattr(auto_tuner, '_meta_governor'):
        del auto_tuner._meta_governor
        print("   🧹 Old meta-governor state purged (API -> internal transition)")
    ...
    # ── v12.0: Governor target recalibration ──
    # The transition ramped targets originally to 150 but norms are at 30-40.
    # Set targets to 1.3× current norms so governors can engage.
    with torch.no_grad():
        sample_ffn_norms = []
        sample_contrib_norms = []
        for block in model.blocks:
            info = getattr(block, '_last_info', {}) or {}
            if 'mean_ffn_norm' in info:
                sample_ffn_norms.append(float(info['mean_ffn_norm']))
            if 'mean_contrib_norm' in info:
                sample_contrib_norms.append(float(info['mean_contrib_norm']))

        if sample_ffn_norms:
            current_ffn = sum(sample_ffn_norms) / len(sample_ffn_norms)
            new_ffn_target = max(20.0, current_ffn * 1.15)
        else:
            new_ffn_target = 50.0

        if sample_contrib_norms:
            current_alpha = sum(sample_contrib_norms) / len(sample_contrib_norms)
            new_alpha_target = max(20.0, current_alpha * 1.15)
        else:
            new_alpha_target = 45.0
        ...
        cfg.recal_ffn_target = new_ffn_target
        cfg.recal_alpha_target = new_alpha_target
        cfg.recal_softcap_target = max(25.0, new_ffn_target * 1.0)
        
        print(f"   🎯 Governor recalibration: FFN target → {new_ffn_target:.0f}, "
              f"Alpha target → {new_alpha_target:.0f}, "
              f"SoftCap → {max(25.0, new_ffn_target * 1.15):.0f}")
    ...
    # For fresh start, use hardcoded bootstrap targets
    RECAL_FFN = 90.0    # ← was 50.0
    RECAL_ALPHA = 55.0  # ← was 45.0
    RECAL_CAP = 120.0   # ← was 75.0
    for block in model.blocks:
        block.ffn_norm_target = RECAL_FFN
        block.alpha_norm_target = RECAL_ALPHA
        block.soft_cap_target = RECAL_CAP
    cfg.governor_recalibrated = True
    cfg.recal_ffn_target = RECAL_FFN
    cfg.recal_alpha_target = RECAL_ALPHA
    cfg.recal_softcap_target = RECAL_CAP
    print(f"   🎯 Governor recalibration: FFN → {RECAL_FFN:.0f}, "
          f"Alpha → {RECAL_ALPHA:.0f}, SoftCap → {RECAL_CAP:.0f}")
...
# =====================================================================
# 🌊 SDE & 🧶 FIBER TELEMETRY OBSERVATORIES
# =====================================================================
_sde_hidden_states = {}
_fiber_q = {}
...
def _sde_hook(layer_idx):
    def hook(module, input, output):
        if isinstance(output, tuple):
            ...
            x = output
        _sde_hidden_states[layer_idx] = x.detach()[:, :64, :]
    return hook

def _fiber_q_hook(layer_idx):
    def hook(module, input, output):
        _fiber_q[layer_idx] = output.detach()[:, :64, :]
    return hook
...
for _idx in _sde_lattice:
    if _idx < len(model.blocks):
        ...
        if hasattr(model.blocks[_idx].attn, 'qkv'):
            model.blocks[_idx].attn.qkv.register_forward_hook(_fiber_q_hook(_idx))
...
def compute_sde_telemetry():
    ...
    for idx in range(len(layers) - 1):
        ...
        h_curr = _sde_hidden_states[l_curr]  # [B, N, D]
        h_next = _sde_hidden_states[l_next]  # [B, N, D]

        # --- RAW (magnitude + direction) ---
        ...
        # --- DIRECTIONAL (direction only) ---
        # Step 1: Normalize hidden states to unit vectors
        u_curr = h_curr / (torch.norm(h_curr, dim=-1, keepdim=True) + 1e-8)
        u_next = h_next / (torch.norm(h_next, dim=-1, keepdim=True) + 1e-8)

        # Step 2: Compute directional increments
        v_dir = u_next - u_curr  # [B, N, D]

        # Step 3: Normalize the increments
...
def compute_fiber_curvature():
    ...
    for l in layers:
        ...
        k_heads = k.view(B, T, H, D_head).transpose(1, 2)
        T_slice = min(T, 64)
        q_slice, k_slice = q_heads[:, :, :T_slice, :], k_heads[:, :, :T_slice, :]
        logits = torch.matmul(q_slice, k_slice.transpose(-2, -1)) / (D_head ** 0.5)
...
for step in tqdm(range(step, total_steps), desc="Training", initial=step):
    ...
    # ── Logging, telemetry, checkpoints every LOG_EVERY ──
    if step % LOG_EVERY == 0 and step > 0:
        ...
        # 🌊 SDE & 🧶 Fiber Telemetry Computation
        sde_metrics = compute_sde_telemetry()
   # --- SDE TELEMETRY AUDIT ---
        print(f"   🔬 SDE AUDIT: drift={sde_metrics.get('sde_drift_norm', 'MISSING'):.4f} | "
          f"diffusion={sde_metrics.get('sde_noise_trace', 'MISSING'):.4f} | "
          f"raw_snr={sde_metrics.get('sde_snr', 'MISSING'):.4f}")
    # ---------------------------
        _info_src["sde_drift_norm"] = sde_metrics["sde_drift_norm"]
        _info_src["sde_noise_trace"] = sde_metrics["sde_noise_trace"]
        _info_src["sde_snr"] = sde_metrics["sde_snr"]   # Raw SNR (ρ_raw)
        _info_src["sde_snr_dir"] = sde_metrics["sde_snr_dir"] # Directional SNR (ρ_dir)
        _info_src["fiber_curvature_head_var"] = compute_fiber_curvature()
     
        # v12.2: Collect fiber curvature from telemetry lattice blocks
        # (Computed during forward pass in MYCELIA_architecture.py)
        TELEMETRY_LATTICE = {4, 8, 12, 16, 20, 23}
        fiber_vars = []
        for idx, block in enumerate(model.blocks):
            if idx in TELEMETRY_LATTICE and hasattr(block, '_last_fiber_curvature'):
                fiber_vars.append(block._last_fiber_curvature)
        if fiber_vars:
            with torch.no_grad():
        ...
        # ── v11.5: ADAPTIVE ALPHA POTENTIAL WELL ──
        if _info_src:
        ...
        if hasattr(model, '_last_info') and model._last_info:
            ...
            # v11.8: MPC I-field baseline tracker
            if not hasattr(auto_tuner, '_i_field_ema'):
                auto_tuner._i_field_ema = 0.70
            ...
            if current_target < adaptive_floor * 0.98:
        ...
        if hasattr(auto_tuner, '_alpha_wake_step') and auto_tuner._alpha_wake_step > 0:
            ...
            # Ease: 150 → 35, using cosine ease-in
            ease = 0.5 - 0.5 * math.cos(math.pi * wake_progress)
            new_alpha_target = 150.0 - ease * (150.0 - 35.0)
            for block in model.blocks:
                block.alpha_norm_target = new_alpha_target
            if step % LOG_EVERY == 0:
                print(f"   🍄 Alpha wake: progress {wake_progress*100:.1f}% | target={new_alpha_target:.1f}")
        ...
        # =====================================================================
        # 🌍 BATCH PROFILE TELEMETRY (Real-time Domain/Language Classification)
        # Uses Qwen vocab heuristics to identify the current batch composition.
        # =====================================================================
        # Adjust 'batch["input_ids"]' if your input tensor variable is named differently
        _batch_ids = input_ids 
        
        # Qwen Vocab Heuristics:
        # CJK (Chinese) tokens are heavily concentrated > 15000
        # Czech/English/Latin tokens are mostly < 8000
        _cjk_ratio = (_batch_ids > 15000).float().mean().item()
        _latin_ratio = (_batch_ids < 8000).float().mean().item()
        
        if _cjk_ratio > 0.25:
            _batch_profile = "🇨🇳 CJK-Heavy (Chinese)"
        elif _latin_ratio > 0.70 and _cjk_ratio < 0.05:
            _batch_profile = "🇨🇿/🇬🇧 Latin-Heavy (EN/CZ)"
        else:
            _batch_profile = "🌐 Mixed/Code/Math"
        print(f"   🌍 Batch Profile: {_batch_profile} (CJK:{_cjk_ratio:.2f} | Latin:{_latin_ratio:.2f})")
        ...
        if hasattr(model.blocks[-1], '_hidden_state'):
            ...
            if spectral_concentration > 0.0:
                ...
                if spectral_concentration > 0.95:
                    print(f"   🚨 EARLY WARNING: Dimensional collapse detected!")
        ...
        if alpha_scale_ratio > 0 or mean_contrib_norm > 0:
            # =====================================================================
            # 🏄‍♂️ SURFER TELEMETRY: Extract the live target being used by the Meta-Governor
            # =====================================================================
            _surfer_target = getattr(model.blocks[0], 'alpha_norm_target', 30.0)
            if hasattr(_surfer_target, 'item'):
                _surfer_target = _surfer_target.item()

            # Print it on its own line so it doesn't break your existing AlphaScale formatting
        ...
        # =====================================================================
        # v12.2: Optimization Response Function (chi_R) Tracker
        # Tracks the topological phase transition (SLT) without adding VRAM overhead.
        # =====================================================================
        ...
        if getattr(auto_tuner, '_chi_state', None) is None:
            auto_tuner._chi_state = {
                'prev_R': current_R_chi,
                'prev_loss': float(current_avg_loss) if np.isfinite(current_avg_loss) else 99.0,
                'prev_lambda_w': current_lambda_w,
                'chi_R_ema': 0.0,
                'prev_chi_R_ema': None,
            }

        s = auto_tuner._chi_state
        delta_R = current_R_chi - s['prev_R']
        delta_loss = (float(current_avg_loss) if np.isfinite(current_avg_loss) else 99.0) - s['prev_loss']
        delta_lambda = current_lambda_w - s['prev_lambda_w']

        # Primary: chi_R = dLoss / dR  (the true Optimization Response Function)

        if abs(delta_R) > 0.01:   # from 1e-8 to prevent these false positives when R flat
            chi_R_raw = delta_loss / delta_R
        else:
            chi_R_raw = 0.0  # Hold steady when R is flat

        # Secondary: gauge susceptibility dR/d(lambda_w) (peaks at critical point)
        if abs(delta_lambda) > 1e-6:
            gauge_sens = delta_R / delta_lambda
        else:
            gauge_sens = 0.0

        # 🛡️ FIX: Actually update the EMA! (This was missing, causing the -0.4618 freeze)
        s['chi_R_ema'] = 0.9 * s['chi_R_ema'] + 0.1 * chi_R_raw
        
        info['optimization_response_chi_R'] = s['chi_R_ema']
        info['gauge_sensitivity_dR_dlambda'] = gauge_sens

        # Detect the SLT Phase Transition Sign Flip
        if s['prev_chi_R_ema'] is not None:
            if s['prev_chi_R_ema'] < 0 and s['chi_R_ema'] > 0:
                print(f"🌌 SLT PHASE TRANSITION: chi_R flipped POSITIVE ({s['prev_chi_R_ema']:+.4f} -> {s['chi_R_ema']:+.4f}). Entering Grokking Window.")
            elif s['prev_chi_R_ema'] > 0 and s['chi_R_ema'] < 0:
                print(f"⚠️  SLT REGRESSION: chi_R flipped NEGATIVE ({s['prev_chi_R_ema']:+.4f} -> {s['chi_R_ema']:+.4f}). Entering Compensatory Degeneracy.")

        # Update state for next tick
        s['prev_R'] = current_R_chi
        s['prev_loss'] = float(current_avg_loss) if np.isfinite(current_avg_loss) else 99.0
        ...
        # --- DUAL-REGIME SDE TELEMETRY ---
        rho_raw = _info_src.get('sde_snr', 0.0)
        rho_dir = _info_src.get('sde_snr_dir', 0.0)
        ...
        print(f"   🧭 Cross-token coherence c_l={sde_metrics.get('directional_coherence', 0.0):.3f}")
    ...
    if R < 0.1 and regime_state == "COMPENSATORY" and not getattr(auto_tuner, '_lr_dampened_this_episode', False):
        ...
        # v12.0: Alpha wake-up with accelerated timer -> resolved in governor_auto_tuner
        # if R < 0.1 and regime_state == "COMPENSATORY":
        #    if not hasattr(auto_tuner, '_alpha_wake_step'):
        #        auto_tuner._alpha_wake_step = step
        #    elif step - auto_tuner._alpha_wake_step > 1000:
        #        current_alpha_target = getattr(model.blocks[0], 'alpha_norm_target', 150.0)
        #        if current_alpha_target > 30.0:
        #            new_target = max(30.0, current_alpha_target * 0.85)
        #            for block in model.blocks:
    ...
    if _mpc_recent < 0.05:
        ...
        model._meta_enrichment = {
            ...
            'alpha_wake_active': (
                ...
                (step - auto_tuner._alpha_wake_step) > 1000
            ),
            ...
            'fiber_curvature_head_var': _info_src.get('fiber_curvature_head_var', 0.0),
            'optimization_response_chi_R': info.get('optimization_response_chi_R', 0.0),
            'gauge_sensitivity_dR_dlambda': info.get('gauge_sensitivity_dR_dlambda', 0.0),
            'mean_alpha_scale': _info_src.get('mean_alpha_scale', 1.0),
            'mean_contrib_norm': _info_src.get('mean_contrib_norm', 0.0),
        }

        # ================================================================
        # META-GOVERNOR --- everything above this line has been corrected
        # ================================================================
        ...
        if hasattr(auto_tuner, '_meta_governor'):
            ...
            cb_icon = "🔴" if status.get('circuit_breaker', False) else "🟢"
            mem = status.get('memory_size', 0)
            pending = status.get('pending_verifications', 0)
            eps = status.get('exploration_rate', 0.0)
            print(f"   📡 Meta-Gov: CB={cb_icon} | memory={mem} | pending={pending} | ε={eps:.2f}")

        # =====================================================================
        # v12.1b: Optimization Response Function (chi_R) Tracker
        # Detects SLT phase transitions by tracking dR/d(lambda_w)
        # =====================================================================
        current_R = R  # R is already computed earlier in this LOG_EVERY block
        current_lambda_w = get_alpha_well_depth(opt)

        prev_R = getattr(auto_tuner, '_prev_R', None)
        prev_lambda_w = getattr(auto_tuner, '_prev_lambda_w', None)
        prev_chi_R = getattr(auto_tuner, '_prev_chi_R', 0.0)

        if prev_R is not None and prev_lambda_w is not None:
            delta_R = current_R - prev_R
            delta_lambda = current_lambda_w - prev_lambda_w

            if abs(delta_lambda) > 1e-6:
                chi_R = delta_R / delta_lambda
            else:
                chi_R = prev_chi_R

            if prev_chi_R < 0 and chi_R > 0:
                print("   🌌 SLT PHASE TRANSITION: chi_R flipped POSITIVE → Grokking Window")
            elif prev_chi_R > 0 and chi_R < 0:
                print("   ⚠️ SLT REGRESSION: chi_R flipped NEGATIVE → Compensatory Degeneracy")

            auto_tuner._prev_chi_R = chi_R

        auto_tuner._prev_R = current_R
        auto_tuner._prev_lambda_w = current_lambda_w
        ...
        # ── v12.0: Hard cap on instability_target ──
        # MUST be the LAST thing in this block. No code path may push
        # instability_target outside [0.40, 0.85] after this point.
        old_val = cfg.instability_target
        for block in model.blocks:
            if hasattr(block, 'instability_target'):
                block.instability_target = min(0.85, max(0.40, block.instability_target))
        cfg.instability_target = min(0.85, max(0.40, cfg.instability_target))
        if abs(cfg.instability_target - old_val) > 1e-6:
            print(f"   🔒 instability_target clamped: {old_val:.3f} → {cfg.instability_target:.3f}")
