...
def compute_sde_telemetry():
    ...
    if len(layers) < 2:
        return {"sde_drift_norm": 0.0, "sde_noise_trace": 0.0, "sde_snr": 0.0}
    clean = {}
    for l in layers:
        h = _sde_hidden_states[l]
        clean[l] = h - h.mean(dim=1, keepdim=True)
    drift_norms, noise_traces = [], []
    for i in range(len(layers) - 1):
        delta = clean[layers[i+1]] - clean[layers[i]]
        D1 = delta.mean(dim=[0, 1])
        drift_norms.append(torch.norm(D1).item())
        centered = delta - D1.unsqueeze(0).unsqueeze(0)
        noise_traces.append(0.5 * (centered ** 2).mean().item())
    avg_drift = sum(drift_norms) / len(drift_norms)
    avg_noise = sum(noise_traces) / len(noise_traces)
    snr = avg_drift / (avg_noise ** 0.5 + 1e-8)
    return {"sde_drift_norm": avg_drift, "sde_noise_trace": avg_noise, "sde_snr": snr}
...
for step in tqdm(range(step, step + 610351), desc="Training", initial=step):
    ...
    if step % LOG_EVERY == 0 and step > 0:
        ...
        _info_src["sde_snr"] = sde_metrics["sde_snr"]
