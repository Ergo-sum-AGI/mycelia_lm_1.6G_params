# ============================================
# MYCELIA LM Architecture (v12.3) — Fibonacci Geometric Resonance
#
# Changes from v10.3:
...
# v12.2: Telemetry lattice for fiber curvature proxy.
# Immutable frozenset — allocated once at module load.
_TELEMETRY_LATTICE = frozenset({4, 8, 12, 16, 20, 23})
...
class MycelialAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, padding_mask: Optional[torch.Tensor] = None, return_heads: bool = True):
        ...
        # ── v12.2: Fiber Curvature Proxy (telemetry lattice) ──
        # Compute pre-softmax QK^T variance across heads on a 128-token slice.
        # VRAM cost: ~2 MB per lattice layer; total ~12 MB for 6 layers.
        if getattr(self, 'layer_idx', -1) in _TELEMETRY_LATTICE and self.training:
            with torch.no_grad():
                q_tel = q[:, :, :128, :]      # [B, H, 128, D]
                k_tel = k[:, :, :128, :]      # [B, H, 128, D]
                raw_logits = torch.matmul(
                    q_tel, k_tel.transpose(-2, -1)
                ) / (q_tel.shape[-1] ** 0.5)  # [B, H, 128, 128]
                # Variance across heads -> scalar (deferred .item() to training loop)
                self._last_fiber_curvature = torch.var(raw_logits, dim=1).mean()
...
class MycelialBlock(nn.Module):
    ...
    def _compute_kuramoto_coherence(self, head_outputs: torch.Tensor) -> float:
        """
        MASSIF-inspired: compute Kuramoto order parameter R_t from head velocities.
        head_outputs: (B, n_heads, T, d_head)
        Returns scalar R_t in [0, 1]. R_t ≈ 1 → synchronized, R_t ≈ 0 → chaotic.
        """
        if head_outputs.shape[2] < 2:
            return 1.0
        # Velocity: v_t = z_t - z_{t-1} for each head
        z_t = head_outputs[:, :, -1, :]      # (B, H, d_head)
        z_prev = head_outputs[:, :, -2, :]   # (B, H, d_head)
        v_t = z_t - z_prev                   # (B, H, d_head)
        # Project onto first 2 dims to get phase angle (MASSIF style)
        v_x = v_t[..., 0].float()            # ← cast to float32
        v_y = v_t[..., 1].float() if v_t.shape[-1] > 1 else torch.zeros_like(v_x)
        theta_t = torch.atan2(v_y, v_x)      # (B, H) float32
        # Kuramoto order parameter
        complex_phases = torch.complex(torch.cos(theta_t), torch.sin(theta_t))
        mean_phase = complex_phases.mean()   # mean over B, H
        R_t = torch.abs(mean_phase)
        return float(R_t.item())

        # MASSIF-inspired kinematics
        # M_n: Normalized Net Displacement (progress)
        # net_displacement = (x - self._prev_hidden).norm(p=2, dim=-1)
        # M_n = net_displacement / max(x.shape[1], 1)  # normalized by seq len

        # M_n from velocity (before buffer update) = more defensive than the above
        net_displacement = velocity.norm(p=2, dim=-1)
        M_n = net_displacement / max(T, 1)

        # K_n: Mean trajectory curvature (stability)
        # Using identity: ||v × a||^2 = ||v||^2 ||a||^2 - (v·a)^2
        v_norm_sq = velocity_norm.pow(2)
        a_norm_sq = acceleration_norm.pow(2)
        dot_va = (velocity * acceleration).sum(dim=-1) if velocity.shape == acceleration.shape else torch.zeros_like(v_norm_sq)
        cross_norm_sq = torch.clamp(v_norm_sq * a_norm_sq - dot_va.pow(2), min=0.0)
        K_n = torch.sqrt(cross_norm_sq) / (v_norm_sq.pow(1.5) + 1e-6)
        K_n = K_n.clamp(max=10.0).mean()

        return velocity_norm, acceleration_norm, curvature, jerk, float(M_n.mean().item()), float(K_n.item())
    ...
    def _compute_geometric_observables(self, x: torch.Tensor):
        ...
        # MASSIF-inspired kinematics (step-wise proxies)
        M_n = float(velocity_norm.mean().item()) / max(T, 1)
        v_norm_sq = (velocity ** 2).sum(dim=-1)
        a_norm_sq = (acceleration ** 2).sum(dim=-1)
        dot_va = (velocity * acceleration).sum(dim=-1)
        cross_norm_sq = torch.clamp(v_norm_sq * a_norm_sq - dot_va.pow(2), min=0.0)
        K_n = (torch.sqrt(cross_norm_sq) / (v_norm_sq.pow(1.5) + 1e-6)).clamp(max=10.0).mean()
        ...
        return velocity_norm, acceleration_norm, curvature, jerk, float(M_n), float(K_n.item())
    ...
    def forward(self, x: torch.Tensor, step: int = 0,
        ...
        if self.config.use_gradual_transition:
            ...
            if progress < 1.0:
                # During transition: use config-based ramp
                ...
                # v12.1: Transition complete — hand over to dynamically adjusted
                # block-level targets (alpha wake-up, meta-governor, auto-tuner)
        ...
        velocity_norm, acceleration_norm, curvature, jerk, M_n, K_n = self._compute_geometric_observables(x)
        ...
        # Consensus rounds iteratively refine the hidden state (progressive refinement).
        # Each round attends to the output of the previous round, not the original residual.
        ...
        for round_idx in range(self.consensus_rounds):
            ...
            kuramoto_R = self._compute_kuramoto_coherence(head_outputs)
            final_round_info['kuramoto_R'] = kuramoto_R
        ...
        final_round_info['M_n'] = M_n
        final_round_info['K_n'] = K_n
